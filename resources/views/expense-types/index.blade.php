...
<x-dashboard-header
    title="Expense Types"
    subtitle="Manage your expense type categories."
/>
...
<div class="max-w-5xl mx-auto p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-3xl font-extrabold text-gray-900">Expense Types</h2>
        <button id="addExpenseTypeBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-md transition">
            Add Expense Type
        </button>
    </div>
...
<div class="bg-white shadow-xl rounded-2xl p-6 ring-1 ring-gray-900/5">
    <div class="overflow-x-auto">
        ...
        <table id="expenseTypeTable" class="min-w-full divide-y divide-gray-200">
            <thead class="bg-indigo-50">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-indigo-700 uppercase tracking-wider rounded-tl-lg">Acronym</th>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex justify-center items-center gap-2">
                                ...
                                <button class="editBtn text-yellow-500 hover:text-yellow-600 p-1 rounded-full hover:bg-yellow-50 transition duration-150 ease-in-out"
                                    ...
                                    data-acronym="{{ $expenseType->acronym }}"
                                    title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>

                                {{-- Delete Form with Icon --}}
                                <form action="{{ route('expense-types.destroy', $expenseType->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="text-red-600 hover:text-red-700 p-1 rounded-full hover:bg-red-50 transition duration-150 ease-in-out"
                                        onclick="return confirm('Are you sure you want to delete {{ $expenseType->description }}?')"
                                        title="Delete">
...
<!-- Modal Overlay -->
<div id="expenseTypeModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6 relative">
        ...
        <!-- Modal Title -->
        <h2 class="text-2xl font-bold mb-4" id="formTitle">Add Expense Type</h2>

        <!-- Form -->
        <form id="formElement" method="POST" action="{{ route('expense-types.store') }}">
            @csrf

            <!-- Description Row -->
            <div class="mb-4">
                <x-input-label value="Description" />
                <x-input type="text" name="description" id="descriptionInput" placeholder="e.g., Travel and Transportation" class="mt-1 block w-full" />
                <x-input-error :messages="$errors->get('description')" class="mt-1" />
            </div>

            <!-- Acronym Row -->
            <div class="mb-4">
                <x-input-label value="Acronym" />
                <x-input type="text" name="acronym" id="acronymInput" placeholder="e.g., TT, OE, PS" class="mt-1 block w-full uppercase" maxlength="10" />
                <x-input-error :messages="$errors->get('acronym')" class="mt-1" />
                <p class="text-xs text-gray-500 mt-1">Short abbreviation (max 10 characters)</p>
            </div>
...
<script>
    ...
    addBtn.addEventListener('click', () => {
        modal.classList.remove('hidden');
        formTitle.textContent = 'Add Expense Type';
        submitBtn.textContent = 'Add';
        formElement.action = "{{ route('expense-types.store') }}";
    ...
    document.querySelectorAll('.editBtn').forEach(button => {
        button.addEventListener('click', () => {
            modal.classList.remove('hidden');
            formTitle.textContent = 'Edit Expense Type';
            submitBtn.textContent = 'Update';
            formElement.action = `/expense-types/${button.dataset.id}`;

            descriptionInput.value = button.dataset.description;
            acronymInput.value = button.dataset.acronym;
