<x-app-layout>
    @section('title', 'Edit Budget Realignment')

    <x-dashboard-header
        title="Edit Budget Realignment"
        subtitle="Modify budget realignment details and items"
    />
    ...
    <div class="max-w-7xl mx-auto p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-3xl font-extrabold text-gray-900">Edit Budget Realignment</h2>
                <p class="text-sm text-gray-600 mt-1">{{ $budgetRealignment->realignment_number }}</p>
            </div>
            <a href="{{ route('budget-realignments.show', $budgetRealignment) }}" 
        ...
        <!-- Form -->
        <form action="{{ route('budget-realignments.update', $budgetRealignment) }}" method="POST" id="realignmentForm">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <textarea name="description" rows="3" 
                                      ...
                                      required>{{ old('description', $budgetRealignment->description) }}</textarea>
                        </div>
                        <div>
                            <x-input-label value="Fiscal Year" />
                            <input type="text" 
                                   class="mt-1 block w-full border border-neutral-300 rounded-lg px-4 py-2 bg-gray-100" 
                                   value="{{ $budgetRealignment->fiscalYear->year ?? 'N/A' }}" 
                                   readonly>
            ...
            <div class="bg-white shadow-xl rounded-2xl overflow-hidden ring-1 ring-gray-900/5 mb-6">
                ...
                <div class="p-6">
                    <div id="fromItemsContainer" class="space-y-4">
                        <!-- From items will be populated here -->
                    </div>
            ...
            <div class="bg-white shadow-xl rounded-2xl overflow-hidden ring-1 ring-gray-900/5 mb-6">
                ...
                <div class="p-6">
                    <div id="toItemsContainer" class="space-y-4">
                        <!-- To items will be populated here -->
                    </div>
    ...
    // Existing items data
    const existingFromItems = @json($budgetRealignment->fromItems->map(function($item) {
        return [
            'department_id' => $item->allocation->department_id,
            'expense_type_id' => $item->allocation->expense_type_id,
            'account_id' => $item->allocation->account_id,
            'sub_account_id' => $item->allocation->sub_account_id,
            'amount' => $item->amount,
            'remarks' => $item->remarks
        ];
    }));

    const existingToItems = @json($budgetRealignment->toItems->map(function($item) {
        return [
            'department_id' => $item->allocation->department_id,
            'expense_type_id' => $item->allocation->expense_type_id,
            'account_id' => $item->allocation->account_id,
    ...
    $(document).ready(function() {
        console.log('Document ready - Budget Realignment Edit');
        ...
        $('#addFromItem').on('click', function() {
            console.log('Add From Item clicked (Edit)');
            addFromItem();
        });
        $('#addToItem').on('click', function() {
            console.log('Add To Item clicked (Edit)');
            addToItem();
        });
    ...
    function loadExistingItems() {
        // Load existing from items
        existingFromItems.forEach(function(item) {
            addFromItem(item);
        });
        
        // Load existing to items
        existingToItems.forEach(function(item) {
            addToItem(item);
        });
        
        // Check allocation info for all existing items after a short delay
        setTimeout(function() {
            $('.from-item, .to-item').each(function() {
                checkAllocationInfo($(this));
            });
        }, 500);
    ...
    function addFromItem(existingData = null) {
        const html = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="from_items[${fromItemIndex}][amount]" step="0.01" min="0.01"
                               class="from-amount-input w-full border border-gray-300 rounded-lg px-3 py-2" 
                               value="${existingData ? existingData.amount : ''}" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Remarks</label>
                        <textarea name="from_items[${fromItemIndex}][remarks]" rows="2"
                                  class="w-full border border-gray-300 rounded-lg px-3 py-2">${existingData ? existingData.remarks || '' : ''}</textarea>
                    </div>
                </div>
        ...
        // Set the values if existing data
        if (existingData) {
            $(`select[name="from_items[${fromItemIndex}][department_id]"]`).val(existingData.department_id);
            $(`select[name="from_items[${fromItemIndex}][expense_type_id]"]`).val(existingData.expense_type_id);
            $(`select[name="from_items[${fromItemIndex}][account_id]"]`).val(existingData.account_id);
            $(`select[name="from_items[${fromItemIndex}][sub_account_id]"]`).val(existingData.sub_account_id);
        }
    ...
    function addToItem(existingData = null) {
        const html = `
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="to_items[${toItemIndex}][amount]" step="0.01" min="0.01"
                               class="to-amount-input w-full border border-gray-300 rounded-lg px-3 py-2" 
                               value="${existingData ? existingData.amount : ''}" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Remarks</label>
        ...
        // Set the values if existing data
        if (existingData) {
            $(`select[name="to_items[${toItemIndex}][department_id]"]`).val(existingData.department_id);
            $(`select[name="to_items[${toItemIndex}][expense_type_id]"]`).val(existingData.expense_type_id);
            $(`select[name="to_items[${toItemIndex}][account_id]"]`).val(existingData.account_id);
            $(`select[name="to_items[${toItemIndex}][sub_account_id]"]`).val(existingData.sub_account_id);
        }
    ...
    function checkAllocationInfo(itemContainer) {
        const fiscalYearId = {{ $budgetRealignment->fiscal_year_id }};
        ...
        if (!departmentId || !expenseTypeId || !accountId) {
